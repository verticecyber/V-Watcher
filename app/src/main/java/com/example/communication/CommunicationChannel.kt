...
class DefaultCommunicationChannel(
  ...
  /**
   * PH-04: bounded ring buffers making emitted traffic queryable without a collector.
   * Observability only; delivery semantics of the flows are unchanged.
   */
  private val _recentTelemetry = MutableStateFlow<List<SentinelTelemetryEvent>>(emptyList())
  val recentTelemetry: StateFlow<List<SentinelTelemetryEvent>> = _recentTelemetry.asStateFlow()

  private val _recentResponses = MutableStateFlow<List<ReasoningResponse>>(emptyList())
  val recentResponses: StateFlow<List<ReasoningResponse>> = _recentResponses.asStateFlow()

  private fun <T> List<T>.cappedAppend(item: T): List<T> = (this + item).takeLast(20)
  ...
  override suspend fun dispatchTelemetry(event: SentinelTelemetryEvent) {
    ...
    _recentTelemetry.update { current -> current.cappedAppend(event) }
  }
  ...
  override suspend fun sendReasoningRequest(request: ReasoningRequest): ReasoningResponse {
    ...
    return try {
      ...
      _recentResponses.update { current -> current.cappedAppend(response) }
      response
    } catch (e: Exception) {
