...
- Static scans: `tools/petscan/static_scan.sh` → `tools/petscan/static_scan_output.txt` (definitions, TODO scan — 1 hit `BatteryProvider.kt:56` default const, thresholds, confidences, payload construction counts, collectors, `startActivity` sites, timings).
...
- Memory: `LocalImmuneMemoryRepository.kt:32-202` (gates `:111-156`, no TTL); stale Room claims in `REAL_DEVICE_VALIDATION_REPORT.md:41,99,115,185`, `CLAIM_PROOF_MATRIX.json:17`, `MEMORY_VALIDATION.json:2`, `LIFECYCLE_VALIDATION.json:11`, `TRUTH_FIRST_VALIDATION_REPORT.md:49-50`.
